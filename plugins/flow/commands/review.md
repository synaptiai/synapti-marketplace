...
|---|---|
| git | The pull request's `.gitattributes` is never applied: the checkout runs with `GIT_ATTR_SOURCE` set to the empty tree (git 2.40 or later), so no filter driver it names runs (git-lfs would contact a host its `.lfsconfig` names), and every dispatch exports the same, so `git grep` and `git diff` see its files as text. `safe.bareRepository=explicit` stops a repository it commits from loading its config. `git -C "$REVIEW_TREE"` at its top only; diffs also take `--text --no-ext-diff --no-textconv` |
| Bash | Never `cd` into the tree. Anything that runs in it sits behind the guard line and needs `REVIEW_RUN_PR_COMMANDS=yes` |
| Read, Grep, Glob | Full paths under `REVIEW_TREE`. The checkout step removes every symlink from the tree, so none of them can reach a file outside it, and every `.ignore`, `.rgignore` and `.gitignore`, which Grep and Glob would otherwise obey and skip files by |
...
if [ "$PR_AUTHOR" = "$CURRENT_USER" ]; then
  ...
  # reads attributes from the empty tree instead; git before 2.40 ignores it.
  case "$(git version)" in
    "git version 1."*|"git version 2."[0-9]"."*|"git version 2."[0-3][0-9]"."*)
      printf '%s\n' "ERROR: git 2.40 or later is needed to check out someone else's pull request without its .gitattributes; this is $(git version)" >&2
      exit 1 ;;
  esac
  EMPTY_TREE=$(git hash-object -t tree -w /dev/null) || { printf '%s\n' "ERROR: cannot write the empty tree" >&2; exit 1; }
  GIT_ATTR_SOURCE="$EMPTY_TREE" git -c core.hooksPath=/dev/null worktree add --quiet --detach "$REVIEW_PARENT/tree" "$HEAD_OID" || { printf '%s\n' "ERROR: cannot add a worktree for the pull request" >&2; exit 1; }
  ...
  if [ "$REVIEW_RUN_PR_COMMANDS" != yes ]; then
    ...
    __ls=$(mktemp "${TMPDIR:-/tmp}/flow-review-ls.XXXXXX") || { printf '%s\n' "ERROR: cannot make a temporary file" >&2; exit 1; }
    git -C "$REVIEW_TREE" ls-files -s -z > "$__ls" || { rm -f "$__ls"; printf '%s\n' "ERROR: cannot list the pull request's files" >&2; exit 1; }
    SYMLINKS_REMOVED=0
    IGNORE_FILES_REMOVED=0
    while IFS= read -r -d '' __e; do
      __p=${__e#*	}
      case "$__e" in
        120000\ *) rm -f -- "$REVIEW_TREE/$__p" && SYMLINKS_REMOVED=$((SYMLINKS_REMOVED + 1)); continue ;;
      esac
      case "/$__p" in
        */.ignore|*/.rgignore|*/.gitignore) rm -f -- "$REVIEW_TREE/$__p" && IGNORE_FILES_REMOVED=$((IGNORE_FILES_REMOVED + 1)) ;;
      esac
    done < "$__ls"
    rm -f "$__ls"
    __left=$(find "$REVIEW_TREE" -type l -print 2>/dev/null | head -1)
    [ -z "$__left" ] || { printf '%s\n' "ERROR: a symlink is still in the pull request's tree: $__left" >&2; exit 1; }
