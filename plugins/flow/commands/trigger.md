...
if [ "$ENABLED" != "true" ]; then
  printf '%s\n' "flow.triggers.enabled is false — /flow:trigger is opt-in." >&2
  printf '%s\n' "Enable in .claude/settings.flow.local.json: { \"flow\": { \"triggers\": { \"enabled\": true } } }"
  exit 0
fi
