...
4. **Produce the Coverage Scan table** (shape in [`references/verdict-output-format.md`](../references/verdict-output-format.md)) BEFORE evaluating anything. The seven mandatory subsections per criterion are `### Does NOT promise` (its own column), `### Visual analysis` (two columns: "Visual Analysis Present?" — `Yes` when a `ui` criterion carries one `Viewport:`/`Screenshot:`/`Result:`/`Observed:` block per configured viewport, `NO` when a `ui` criterion has it missing, blank, `none`, or without `Observed:` sentences, `N/A` when a non-`ui` criterion carries `none — criterion type {type} has no visual surface`; and "Interaction Steps Present?" — `Yes` when a `ui` criterion describing a user action carries at least one `Step:` block, `NO` when it carries neither a `Step:` block nor a conforming `Flows: none — {reason}` line, `N/A (no flow: {reason})` when it carries the line, `N/A` otherwise), and the five completeness subsections checked by the "Completeness Subsections Present?" column: `### What was NOT tested`, `### Known limitations of this evidence`, `### Negative/adversarial cases covered`, `### Test inputs and expected values`, `### Risk map coverage`. `none` is a positive statement and counts as present; blank or omitted counts as missing. The "Holdout Validation Status" column is PASS (no conflict), CONFLICT (holdout-validation found a P1/P2 for this criterion), or N/A.
5. **Automatic FAIL rules** — apply before per-criterion evaluation and record the result:
   ...
   - `### Type` is `ui`, the criterion describes an action a user performs (cue verbs: click, submit, type, select, toggle, open, navigate, drag), `### Visual analysis` carries no `Step:` block, AND it carries no `Flows: none — {reason}` line → FAIL, rationale `incomplete evidence — missing interaction steps on a ui criterion`. A `Flows:` line is the producer stating why no flow ran, and its `{reason}` is one of exactly three: `visualVerification.flows=off`, `no interactive tool`, or `no interaction`. Anything else — including a bare `Flows: none` with no reason — is `producer non-conforming` and does NOT suppress this auto-FAIL; otherwise any text at all would. You receive no settings and cannot determine why a flow was skipped yourself, which is why the producer has to say.
...
6. On a `ui` criterion, read `### Visual analysis` and apply:
   ...
   - **(e) Interaction steps.** A block carrying a `Step: {n}/{m} {action}` line is one step of a driven flow, not a viewport. It does not satisfy rule (d)'s viewport coverage and rule (d) does not demand one per viewport — the two are counted separately.
     - **Whenever step blocks are present**, however the flow was triggered: every `Step:` block must report `Result: PASS` with `Observed:` text describing the page after that step. Any `Step:` block reporting `Result: FAIL`, or whose `Observed:` text does not describe the page after that step → FAIL, rationale `visual analysis does not show required state`. A flow may also be triggered by a risk-map row rather than the criterion's wording, so this clause is deliberately not conditioned on a cue verb: a failing step is a failing step.
     - **When the criterion describes an action a user performs** (cue verbs: click, submit, type, select, toggle, open, navigate, drag), at least one `Step:` block must be present, or a conforming `Flows: none — {reason}` line explaining why none is. When the criterion itself mentions responsive or mobile, the flow was meant to run on every configured viewport, so there must be a `Step:` block for each — or a `Flows:` line naming the viewports left undriven. A mobile criterion evidenced by a desktop-only flow has not been verified where it claims to apply. Neither → FAIL under Step 1, rationale `incomplete evidence — missing interaction steps on a ui criterion`: a screenshot of the page before the interaction is not evidence that the interaction works.
...
- Test output showing pass for the exact behavior, `### Visual analysis` whose every viewport block is `Result: PASS` with `Observed:` text naming the required state, curl response matching expected status/body → PASS (subject to rules a-e)
...
|---|---|---|
| **PASS** | Evidence directly confirms the criterion | Test passes for the exact behavior with independent expected values and discriminating inputs; every `### Visual analysis` viewport block and every `Step:` block is `Result: PASS` and its `Observed:` text names the required state; API returns expected response |
