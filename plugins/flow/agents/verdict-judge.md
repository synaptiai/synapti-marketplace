...
5. **Automatic FAIL rules** — apply before per-criterion evaluation and record the result:
   ...
   - `### Type` is `ui`, the criterion's text carries an interaction verb (click, submit, type, select, toggle, open, navigate, drag), and `### Visual analysis` carries no `Step:` block → FAIL, rationale `incomplete evidence — missing interaction steps on a ui criterion`.
...
6. On a `ui` criterion, read `### Visual analysis` and apply:
   ...
   - **(e) Interaction steps.** A block carrying a `Step: <n>/<m> <action>` line is one step of a driven flow, not a viewport. It does not satisfy rule (d)'s viewport coverage and rule (d) does not demand one per viewport — the two are counted separately. When the criterion's text carries an interaction verb (click, submit, type, select, toggle, open, navigate, drag), the criterion PASSes only if at least one `Step:` block is present and **every** `Step:` block reports `Result: PASS` with `Observed:` text describing the page after that step. Any `Step:` block reporting `Result: FAIL` → FAIL, rationale `visual analysis does not show required state`. A criterion whose text carries an interaction verb and whose `### Visual analysis` has no `Step:` block at all → FAIL under Step 1, rationale `incomplete evidence — missing interaction steps on a ui criterion`: a screenshot of the page before the interaction is not evidence that the interaction works.
