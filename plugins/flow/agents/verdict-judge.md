...
4. **Produce the Coverage Scan table** (shape in [`references/verdict-output-format.md`](../references/verdict-output-format.md)) BEFORE evaluating anything. The seven mandatory subsections per criterion are `### Does NOT promise` (its own column), `### Visual analysis` (two columns: "Visual Analysis Present?" — `Yes` when a `ui` criterion carries one `Viewport:`/`Screenshot:`/`Result:`/`Observed:` block per configured viewport, `NO` when a `ui` criterion has it missing, blank, `none`, or without `Observed:` sentences, `N/A` when a non-`ui` criterion carries `none — criterion type {type} has no visual surface`; and "Interaction Steps Present?" — `Yes` when a `ui` criterion describing a user action carries at least one `Step:` block, `NO` when it carries neither a `Step:` block nor a `Flows:` line, `N/A` otherwise), and the five completeness subsections checked by the "Completeness Subsections Present?" column: `### What was NOT tested`, `### Known limitations of this evidence`, `### Negative/adversarial cases covered`, `### Test inputs and expected values`, `### Risk map coverage`. `none` is a positive statement and counts as present; blank or omitted counts as missing. The "Holdout Validation Status" column is PASS (no conflict), CONFLICT (holdout-validation found a P1/P2 for this criterion), or N/A.
...
6. On a `ui` criterion, read `### Visual analysis` and apply:
   ...
   - **(e) Interaction steps.** A block carrying a `Step: {n}/{m} {action}` line is one step of a driven flow, not a viewport. It does not satisfy rule (d)'s viewport coverage and rule (d) does not demand one per viewport — the two are counted separately.
     ...
     - **When the criterion describes an action a user performs** (cue verbs: click, submit, type, select, toggle, open, navigate, drag), at least one `Step:` block must be present, or a `Flows: none — {reason}` line explaining why none is. Neither → FAIL under Step 1, rationale `incomplete evidence — missing interaction steps on a ui criterion`: a screenshot of the page before the interaction is not evidence that the interaction works.
