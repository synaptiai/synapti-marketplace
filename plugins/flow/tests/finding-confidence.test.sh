...
#   - The review.md routing and posting blocks, extracted and run with a
#     stubbed gh under bash and zsh, keep LOW-confidence findings out of the
#     FLOW_REVIEW_CYCLE marker and the counts, and refuse a body whose counts,
#     sections or marker disagree with the routed findings.
#   - The review-mode, dropped-finding, review-cycle manifest, grounding and
#     stranger-test blocks refuse empty, forged or malformed input, and record
#     what they say they record.
#   - Every issue lookup asks GitHub for the closing issue rather than parsing
#     a number out of pull request text, and no command records an unquoted
#     metadata value.
#   - Bodies rendered from the real templates pass the posting guards.
...
assert_contains "never in the bold" "$(_fc_phase4_step 7)" "step 7 tells the reviewer how to write prior-cycle ids"
