...
### Companion fixes (surfaced during PR review of #101)

- **`commands/status.md` markerTrust gate missed the same fix.** Surfaced by the parallel-reviewer pass during `/flow:pr` for #101. `commands/status.md:67-77` read `merge.markerTrust.allowedAssociations` from plugin tier only, identical to the broken pattern in `commands/merge.md`. Now applies the same trimmed cascade — without this fix, a marketplace user setting `merge.markerTrust.allowedAssociations` in `$HOME` would have it honored by `/flow:merge` but silently ignored by `/flow:status`, producing inconsistent ledger state between the two commands.
- **`commands/merge.md` markerTrust gate now surfaces JSON parse errors.** Previously `2>/dev/null` silently swallowed parse errors in the source file, treating malformed JSON identically to "key absent" → fall through to plugin default with no warning. Now mirrors the agentTeams gate's per-source `WARN: failed to parse $SETTINGS_PATH (jq exit=$JQ_EXIT, error: ...)` so a typo in `$HOME/.claude/settings.flow.json` does not silently mask the user's intent.
- **`commands/setup.md` aligned with the trimmed cascade.** Previous setup wrote `agentTeams` and `conventions.commitTypes` into `.claude/settings.flow.json` (project-tier), where they are silently ignored by their consumers. Both keys are trimmed-cascade and need `$HOME/.claude/settings.flow.json` to take effect. New Phase 2.5 explicitly walks the user through creating or updating the user-tier file with the five trimmed-cascade keys (`agentTeams`, `merge.markerTrust.allowedAssociations`, `conventions.commitTypes`, `journal.dir`, `learning.proposalDir`), reminds them about the `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` env-var requirement for `agentTeams: true`, and surfaces any pre-existing project-tier instances of these keys via `AskUserQuestion` for migration. Phase 2 explicitly enumerates which keys are project-tier-safe to write. Phase 6 summary updated to distinguish project-tier from user-tier configuration. Tier classification table grew the user-tier write step (Tier 2: asks first) and the migrate step (Tier 2).
- **`${HOME:-/nonexistent}` defensive default** applied at every USER_SETTINGS construction site (review.md, merge.md, status.md). Catches `set -u` / `env -i` invocations where `$HOME` could otherwise abort the gate with "unbound variable" instead of degrading to the safe "user-tier file does not exist" path.

### New regression tests

- `tests/agentteams-gate/test.sh` (new, 19 assertions) extracts the gate body from `commands/review.md` via `# AGENTTEAMS_GATE_BEGIN` / `# AGENTTEAMS_GATE_END` markers and runs it against scenarios for marketplace install, upgrade survival, project-tier bypass attempts, three-state diagnostic, HOME-overrides-plugin precedence (including opt-OUT semantics), malformed-HOME fall-through to plugin tier, and the env-var double-key requirement. The harness now also runs `bash -n` against the extracted body so a corrupted END marker (e.g. linter reformat that captures past the closing fence) FATALs instead of silently partial-eval'ing.
- `tests/markertrust-gate/test.sh` (new, 10 assertions) extracts the gate body from `commands/merge.md` via `# MARKERTRUST_GATE_BEGIN` / `# MARKERTRUST_GATE_END` markers and covers the same scenarios for `merge.markerTrust.allowedAssociations`, including the empty-array `FINDING_LEDGER_BLOCK` fall-through behavior. Same `bash -n` syntax check applied.
