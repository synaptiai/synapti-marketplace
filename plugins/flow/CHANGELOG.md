...
- **JSON Schema regex portability.** `\Z` anchor (Python-only) replaced with `(?!\n)$` lookahead in `plugins/flow/schemas/holdout-validation/input-schema.json` and `tests/finding-schema/row-schema.json`. The lookahead form is portable across Python `re` and ECMA-262 validators (ajv, etc.), unblocking future use of the schemas in non-Python tooling. All 21 trailing-newline regression assertions still pass.
...
Test totals on this release: 75 assertions across `tests/issue-86/` (16), `tests/skills/{holdout-validation,criterion-verification-map,specification-capture}/` (19), `tests/finding-schema/` (14), `tests/status-parser/` (10), and `tests/journal-orchestration/` (16). All pass — including after the schema relocation, the `(?!\n)$` lookahead migration, and the journal-record concurrency lock (orchestration test still passes 16/16). The synthetic-issue end-to-end manifest journey via Claude Code's prompt layer is a manual integration check exercised by maintainers running `/flow:start` after the helper scripts ship.
