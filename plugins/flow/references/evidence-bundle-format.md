...
When the criterion describes an action a user performs (cue verbs: click, submit, type, select, toggle, open, navigate, drag), or a risk-map row for it names a UI state, and `visualVerification.flows` is `on` with an interactive browser tool available, the section ALSO carries one block per step of each driven scenario. When no flow ran for this criterion, it carries a single `Flows: none — {reason}` line naming why (`visualVerification.flows=off`, `no interactive tool`, or `no interaction`). The two are exclusive on one criterion: either the flow ran and the section carries its step blocks, or it did not and the section carries the line. A scenario that stopped at the step bound carries the blocks it completed, not a `Flows:` line — the line means no flow ran at all. A step block is the same shape with one line added between `Screenshot:` and `Result:`:

```
Step: {n}/{m} {action}
```

Step blocks are additional to the viewport blocks, not a substitute: the viewport blocks prove the page renders across every configured viewport, and the step blocks prove the interaction the criterion describes behaves. A criterion describing a user action whose section has neither a `Step:` block nor a `Flows:` line is an auto-FAIL (`incomplete evidence — missing interaction steps on a ui criterion`), because a screenshot of the page before the interaction is not evidence that the interaction works. A `Flows:` line satisfies it: the producer has said why no flow ran, which is a fact the judge cannot establish for itself.
...
| `### Visual analysis` | **YES (auto-FAIL if missing or blank on a `ui` criterion; `none — criterion type {type} has no visual surface` accepted on every other type; on a `ui` criterion describing an action a user performs, auto-FAIL when it carries neither a `Step:` block nor a `Flows: none — {reason}` line)** | Verdict-judge Step 1 rules: "missing-visual-analysis" and "missing interaction steps"; Step 2 rule (d) reads its viewport blocks and rule (e) its `Step:` blocks |
...
3a. **Copy the visual analysis.** For a `ui` criterion, take the `visual-verification` skill's result tasks (`Visual verification`, `Responsive check`) and copy, per configured viewport, the `Viewport:`/`Screenshot:`/`Result:`/`Observed:` block into `### Visual analysis` exactly as the skill produced it. **Copy every `Step:` block the skill produced as well**, in order — those are the interaction flows, and a bundle that drops them makes the judge auto-FAIL the very criterion they verify. When the skill ran no flow, copy its `Flows: none — {reason}` line instead; without it the judge cannot tell a skipped flow from an omitted one, because it never sees the settings. The producer copies the analysis text; it never asks the judge to look at the file, and it never writes its own description from memory of the screen. If the skill returned no blocks (SKIP, SKIP_WARN, SKIP_USER_APPROVED, MANUAL, BLOCKED) on a `ui` criterion, escalate per `references/escalation-format.md`. For every other type write `none — criterion type {type} has no visual surface`.
