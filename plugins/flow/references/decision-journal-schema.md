...
| `dropped-finding` | `cycle: <int>`, `finding_id: <string>`, `facet: <string>`, `reason: <string>`, `category: <string>` (grounding-pass and step-5 drops), `pr: <int>` | `review.md` Path A A.4 (DROPPED rows); `review.md` Phase 4 step 5 and `pr.md` step 13 (a LOW finding refuted on the author's own PR); the grounding pass in `review.md` (`DROPPED_FINDING_BLOCK` with `REASON`) and `pr.md` (`GROUNDING_DROPS`) | review.md A.4, review.md Phase 4 step 5, pr.md |
...
`dropped-finding` reason values: Path A A.4 records the two DISAGREE reasons as free text. `critic-evidence` marks a finding the originating reviewer dropped after the `finding-critic` answered `DISAGREE_EVIDENCE` with a `file:line`; `critic-unrefuted-concern` marks one dropped because the reviewer could not cite code confirming the bug against a `DISAGREE_CONCERN`. Both come from the grounding pass (`review.groundingCritic: on`). The artifact records the reason, not the critic's line: the line is shown next to the drop in the posted review's "Dropped by the grounding pass" section. Neither is ever recorded for a security finding — one raised by `security-reviewer`, with an id starting `SEC-` or `DEP-`, or with a security category (`security`, `dependency`, `auth`, `injection`, `xss`, `idor`, `secrets`): the grounding pass keeps those, with the critic's line shown beside them, and both record steps refuse such a drop. The same two strings are also `finding-dismissed` reasons, where they mean a human-facing dismissal in `/flow:address`; the record type says which happened. `self-review-refuted` marks a LOW-confidence finding on the author's own pull request that a test or command refuted by passing on the current code; the test stays in the PR and its output is the evidence, and `facet` names the reviewer agent that raised the finding.
