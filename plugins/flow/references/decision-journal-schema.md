...
`pr` is written as an integer by `bin/journal-record.sh`, and `bin/_journal_manifest.py` — the reader behind `/flow:address` step 9 and `/flow:learn` — also accepts a plain digit string, for a journal written by some other route. Nothing else: a float truncates, and `yes` is a YAML boolean whose `int()` is 1, so both would join a dismissal to a pull request that does not name it. A value outside those two shapes makes the whole read unavailable rather than dropping the row, because a dismissal that cannot be placed is not one that belongs elsewhere.
