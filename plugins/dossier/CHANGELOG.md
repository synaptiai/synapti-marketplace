...
- `dossier-scaffold.sh` overwrote a canonical-path file that was empty or missing its frontmatter fence (as designed, to repair a killed mid-write), but reported it as `CREATED` and counted it in `SCAFFOLD_CREATED` — indistinguishable from a clean first scaffold, so a run that destroyed hand-placed content at a canonical path produced a summary that looked identical to one that touched nothing. A new `SCAFFOLD_REPAIRED` counter and `REPAIRED <path>` action line now carry that signal on their own, and the repair is no longer double-counted as a creation. The repair also now names the file and its replaced byte count on stderr. The script's header comment and the `doc-package-contract` skill's matching claim, both of which stated scaffolding "never overwrites" an existing file, now describe the actual fence-conditional behavior.
