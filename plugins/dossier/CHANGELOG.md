...
- `/dossier:audit` Phase 3 requires each verification pass's findings table to be collected into `07-verification/documentation-verification-report.md` unmodified, but the auditors' own analytical prose routinely tripped G18's hard-category checks — the more thoroughly a package was audited, the more likely it was to fail on the auditors' own words rather than the package's. `dossier-prose-lint.sh` now recognizes an explicit `DOSSIER_VERBATIM_BEGIN`/`END` marker pair and exempts text between them from every hard category, but only when scanning `07-verification/documentation-verification-report.md` itself; the markers are inert everywhere else, so a document cannot dodge G18 by wrapping its own prose in them. `/dossier:audit` Phase 5 now emits the markers around each pass's appended section. `--json` reports `verbatim_blocks` and `verbatim_lines_skipped` whenever a marker pair is honored, and an unclosed or nested `BEGIN` is a scan error rather than a silent exemption.
