...
`bin/dossier-vuln-evidence.sh` normalizes vulnerability-scan output (SARIF, osv-scanner JSON, a Dependabot alerts export) into rows following this grammar, so `bin/dossier-gate.sh`'s G19 condition can find them mechanically without a strict column-position parser — the same loose-grep convention G03 already uses. The scan-artifact file it reads may already have existed in the project, or may have just been produced by `bin/dossier-scan-security.sh`'s isolated scan job — `dossier-vuln-evidence.sh` itself only ever reads and normalizes a file already on disk; it never executes a scanner or reaches the network.

- **State**: always `R` (Reported), for both origins above. Whether the scan artifact was already present or was just produced by dossier's own isolated scan job, dossier is reporting what the scanner found — it does not independently re-verify a finding's accuracy against a second source. Executing the scan is not the same as verifying the finding, so executed-scan rows are not promoted to a higher-confidence state than cited-artifact rows.
