...
**The verbatim-collection exemption.** `/dossier:audit` Phase 3 requires each verification pass's findings table to be collected into `07-verification/documentation-verification-report.md` unmodified — reordering, summarizing, or rewording it would corrupt the audit trail G17 depends on. That collected prose is written by the pass agents, not by a document author trying to satisfy G18, and would otherwise fail this condition on its own analytical style (long sentences, semicolons) — failing hardest on exactly the packages that were audited most thoroughly. `bin/dossier-prose-lint.sh` resolves this by recognizing an explicit `<!-- DOSSIER_VERBATIM_BEGIN -->` / `<!-- DOSSIER_VERBATIM_END -->` marker pair and excluding every hard category for text between them — but **only** when scanning the single canonical `07-verification/documentation-verification-report.md` path itself (anchored to that exact path under `--output-root`, so a decoy file elsewhere in the tree cannot claim the exemption by sharing its filename); the same markers are inert HTML comments in every other one of the 23 canonical files, so no *other* document can dodge G18 by wrapping its own prose in them. The exemption still trusts that whatever sits between the markers in the one file it does honor was genuinely written by Phase 3 — it is not a content check, and a document author with write access to that specific file could wrap arbitrary prose in the markers there. G17's own independence requirements are what keep that file's content trustworthy, not this linter. `/dossier:audit` Phase 5 emits the markers around each pass's appended section (see `commands/audit.md`); the round heading and the mechanical summary block stay outside them and are checked like ordinary prose. The linter's `--json` output reports `verbatim_blocks` and `verbatim_lines_skipped` whenever a marker pair is honored, and treats a malformed pair (unclosed or nested `BEGIN`) as a scan error rather than a silent exemption — see the header comment of `bin/dossier-prose-lint.sh` for the exact contract. This exemption is specific to `dossier-prose-lint.sh` — no other dossier check honors these markers. `bin/dossier-claim-scan.sh`'s disclosure scan does not run over `07-verification/` at all under the gate (G06 scans only `06-public/*.md`), so it is not a check the verbatim-marked content passes through either way.
