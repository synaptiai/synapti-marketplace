...
allowed-tools: Bash, Read, Write, Edit, AskUserQuestion, TaskCreate, TaskList, TaskUpdate, TaskGet, Skill, Grep, Glob
---
...
### Step 0.1: Read Conflict Resolution Settings
...
## Phase 1: Determine Invocation Mode

### Step 1.1: Normalize Input

`$ARGUMENTS` can be empty, a PR number, or a branch name:
...
### Step 1.2: Check for Active Merge/Rebase State
...
### Step 1.3: Route by Mode

- **No args + ACTIVE_CONFLICT** — resolve in-progress merge/rebase (skip to Phase 2)
- **No args + NO_ACTIVE_CONFLICT** — report "No active merge conflicts found" and exit
- **PR number** — continue to Step 1.4
- **Branch name** — continue to Step 1.5

### Step 1.4: Check for Uncommitted Changes

If no active conflict, check for uncommitted work:

```bash
git status --porcelain
```

If non-empty, use the **AskUserQuestion tool**:
...
### Step 1.5: PR Mode — Fetch and Attempt Merge
...
### Step 1.6: Branch Mode — Fetch and Attempt Merge

If `$ARGUMENTS` is a branch name:

```bash
# Detect default branch dynamically — never assume main or master
...
If empty, report "No conflicts found between `$ARGUMENTS` and `$DEFAULT_BRANCH`" and complete the merge: `git commit --no-edit`.

## Phase 2: Explore Conflicts

### Step 2.1: List and Classify Conflicted Files

**Execute in parallel** (single message, multiple tool calls):

**2.1a. List conflicted files and classify types:**
...
**2.1b. Count conflict hunks per file:**
...
This is the mandatory gate before completion. All checks must pass.

### Step 5.1: Orphaned Marker Audit
...
If any markers found, go back to Phase 4 for those files.

### Step 5.2: Complete Merge/Rebase
...
### Step 5.3: Quality Verification

Run quality commands discovered in Step 2.2 in parallel. Apply a bounded fix-verify loop:
...
2. **If ALL pass** — proceed to Step 5.4
...
- **Use the AskUserQuestion tool** for:
  ...
  - Uncommitted changes blocking resolution
