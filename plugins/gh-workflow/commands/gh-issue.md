...
1. **Fetch available labels dynamically** (never hardcode):
   ```bash
   gh label list
   ```
...
2. **Use the AskUserQuestion tool** to confirm labels:
   ...
   **Note**: Labels vary by repository. Always fetch and present actual available labels.
...
3. **Report success** with:
   - Issue URL
   - Issue number
   - Next step: `/gh-workflow:gh-start {issue-number}` to begin work
...
1. Use `/gh-workflow:gh-start {issue-number}` to begin work
...
- [ ] Labels selected and confirmed (from dynamically fetched list)
