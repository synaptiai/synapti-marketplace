...
| `python3` with `pyyaml` | 3.x | `python3 -m pip install --user --break-system-packages pyyaml`. Read "Which `python3`" below first. Ledger row requested | **Required.** flow's journal, FlowRun state and FlowGoal machinery need it at runtime [EV-0189]. It also enables the YAML-parse leg of dossier's workflow test [EV-0107] | [EV-0186], [EV-0187], [EV-0189] |
...
Apple's system `bash` 3.2.57 is enough. Both plugin suites pass on it [EV-0098], [EV-0107]. Do not install a newer `bash` to run them.
...
flow's scripts invoke bare `python3`, so the interpreter is whatever `command -v python3` returns. `plugins/flow/bin/journal-record.sh:105` and `:126` both call it that way. Establish yours before installing anything. Ledger row requested.
...
On the drafting machine that resolved to Homebrew's python 3.14.6, and `pip` was not on `PATH` at all. Under that interpreter `python3 -m pip install --user pyyaml` is refused, like this:
...
That refusal is PEP 668, not a broken installation. Adding `--break-system-packages` is accepted, and it is the flag `flow-tests.yml` and `dossier-tests.yml` already use.

Apple's `/usr/bin/python3` accepts `--user` without the flag. Installing there is still wrong while Homebrew's `python3` comes first on `PATH`, because flow's scripts then never reach Apple's interpreter.

The flag needs pip 23.0 or later. Check with `python3 -m pip --version` before relying on it, and drop the flag if the interpreter is old enough to predate PEP 668 anyway.

Scope: observed on macOS 25.6 on 2026-09-10, on one machine. `python3` was Homebrew 3.14.6 with pip 26.1.2, and `/usr/bin/python3` was 3.9 with a user-site pip 26.0.1 rather than its bundled one. Both commands were run as `--dry-run` and reported PyYAML already present, so the flag is accepted and a fresh install remains unexercised. Ledger rows requested.

## Setup

Setup is a clone, three test runs, and one version check. There is no build step, no dependency install, and no environment file [EV-0044].
...
| 3 — run the flow suite | `plugins/flow/tests/run.sh` | `TOTAL pass=… fail=0`, from a checkout at a short absolute path | executed: `TOTAL pass=2336 fail=0`, exit 0 [EV-0098]. A deep checkout gives `pass=2335 fail=1` — see Troubleshooting. Ledger row requested | macOS 25.6, `/bin/bash` 3.2.57 | 2026-09-10 |
...
| 5 — run the repository-root checks (see the testing document for what each asserts) | `for t in tests/*/test.sh tests/*/verify.sh tests/*/validate.sh tests/skills/*/test.sh; do bash "$t"; done` | Each script prints its own PASS / FAIL total | executed: 10 of the 11 exit 0. `tests/agentteams-gate/test.sh` exits 1 at 21 PASS / 3 FAIL. Ledger row requested | macOS 25.6, `/bin/bash` 3.2.57 | 2026-09-10 |
| 6 — check advertised versions | `bash scripts/check-plugin-versions.sh` | `8 plugin(s) checked, 0 failed, 0 unverifiable` | executed, exit 0 [EV-0127] | macOS 25.6, authenticated `gh` | 2026-09-10 |
...
Step 6 needs an authenticated `gh`. Six entries resolve from local paths, and two external entries are read at their pinned sha [EV-0127]. Without network access the script cannot check those two.

**Measured setup time.** Steps 3 and 4 took 2 minutes 20 seconds and 4 minutes 52 seconds. Step 5 took 8 seconds and step 6 took 2 seconds. Measured on macOS 25.6 (Darwin arm64) on 2026-09-10, in a clone with no prior state. Total is about 8 minutes including the clone.
...
`.claude/settings.dossier.json` pins `dossier.ci.expectedPluginVersion` to `1.2.0`, which is the dossier plugin's own version [EV-0139]. It read `1.0.0` until commit `d5a5166` of this refresh corrected it [EV-0139], [EV-0085]. `bin/dossier-validate-config.sh` reports the file valid with zero findings against either value, so nothing would have caught the drift [EV-0086]. A newcomer editing dossier has to compare the two by hand.
...
| Test | `plugins/flow/tests/run.sh` and `plugins/dossier/tests/run.sh` | `TOTAL pass=<n> fail=0` | executed: 2336 and 1951, zero failures, from a short checkout path [EV-0098], [EV-0107] | 2026-09-10 |
| Test the repository-root checks | Each script under `tests/` in turn — see setup step 5 | Each prints its own PASS / FAIL total | executed: 10 of 11 exit 0, `agentteams-gate` exits 1. Ledger row requested | 2026-09-10 |
...
|---|---|---|
| First day | Both plugin suites run green locally, and the version check passes | `pass=2336 fail=0` from a short checkout path, `pass=1951 fail=0`, and `0 failed` on your own machine |
...
| A flow command that records journal, FlowRun or FlowGoal data does not do its work | `pyyaml` is missing. flow's journal and run-state machinery requires it, and no file in the repository declares it [EV-0186], [EV-0187], [EV-0189] | `python3 -m pip install --user --break-system-packages pyyaml`, then re-run the command. Ledger row requested | yes [EV-0189] |
| `pip: command not found` | Only `pip3` and `python3 -m pip` are on `PATH` here. There is no bare `pip` | Use `python3 -m pip`, which also targets the interpreter flow's scripts use. Ledger row requested | yes — `which pip` returned nothing on macOS 25.6 |
| `python3 -m pip install --user pyyaml` prints `error: externally-managed-environment` | PEP 668. Homebrew's interpreter refuses installs into its own tree | Re-run with `--break-system-packages`, or build a virtualenv. Ledger row requested | yes — refusal observed under Homebrew python 3.14.6. Apple's `/usr/bin/python3` accepted `--user` without the flag |
| The flow suite reports `fail=1` in `flow-mine-corrections.test.sh` | Your checkout sits at a long absolute path. The fixture path is cut at 200 characters before the assertion reads it | Move or symlink the checkout to a shorter root. Nothing in the plugin is broken. Ledger row requested | yes — a 163-character root gave `TOTAL pass=2335 fail=1` |
| `tests/agentteams-gate/test.sh` exits 1 | Three of its 24 assertions fail at `b1e716e`: `S4c`, `S6`, `S7b`. No workflow runs this directory | Nothing to fix in your clone. This is the current state of the tree. Ledger row requested | yes — reproduced at 21 PASS / 3 FAIL in a clean detached worktree |
...
|---|---|
| Engineering | The change is made. A test that fails without it exists. Both plugin suites pass |
| Engineering | If the change touches flow's settings cascade, marker parsers, journal helper or logging hooks, the matching script under the repository-root `tests/` also passes. `agentteams-gate` already fails at `b1e716e`, so compare against that baseline rather than against zero. Ledger row requested |
...
| Product | The decision is recorded in `.decisions/`, the only durable mechanism present [EV-0169] |
