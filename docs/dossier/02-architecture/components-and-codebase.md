...
| Root-level drafts: `flow_plugin_medium_article_grounded.md`, `Provably_correct_code_with_Flow_AI_agents_eng.txt` | orphaned | no | unknown | Untracked working-tree files, not part of the repository | working tree at `fd884b2` |
...
| `plugins/dossier` | Daniel Bentes | 1 | 1 | 1227 assertions, one reader | [EV-0009], [EV-0035] |
| `.github/workflows` | Daniel Bentes | 1 | 1 | A workflow change lands with no second opinion and no required check | [EV-0016], [EV-0035] |

The inspected history window is the full repository history across all refs: 364 commits, first 2025-12-19, most recent 2026-07-26 [EV-0034], [EV-0035]. Contributor counts describe that window, not who understands the code today — but with a single identity across the whole window, the two coincide.
