...
| The three facts that drive it | **(1)** Both shipped test suites pass, on macOS at `af6e632` and on Linux CI. flow reports 2336 assertions and 0 failures [EV-0098]. dossier reports 1951 and 0 [EV-0107]. Every Actions run across this range concluded success on `ubuntu-latest` [EV-0126]. **(2)** No dependency-vulnerability scan output exists anywhere in the repository [EV-0121]. **(3)** `main` carries 214 commits under one author identity [EV-0163]. Across all refs, 480 commits carry two identities. The second is an assistant co-author trailer on 22 non-`main` commits, not a third-party contributor [EV-0192]. No component has a backup owner |
...
Both suites pass at `af6e632` [EV-0098], [EV-0107]. No executable file under `plugins/` changed between `af6e632` and the assessed tree `7ee4923`. The one difference is a single line in flow's README [EV-0143], [EV-0174]. The two external plugin sources are pinned to shas [EV-0058].
...
| Project version assessed | `7ee4923` on branch `docs/dossier-refresh`, **not `main`** (AQ-0010). Rows executed at earlier commits in this range are carried to `7ee4923` only where `plugins/` is blob-identical to it [EV-0143], [EV-0174]. The prior HEAD `15bcb24` is not an ancestor of this branch [EV-0118], so rows pinned there are cited as recorded rather than as reachable history |
...
| Evidence obtained outside that ceiling | Three rows rest on GitHub reads the ceiling forbids the agent to make. The session operator ran them directly: Linux CI results [EV-0126], the manifest version check [EV-0127], and the live re-check of branch protection, rulesets and Actions secrets [EV-0131]. This is recorded as AQ-0012 and is not presented as agent-collected |
...
| Claims carried forward without re-checking | README drift, open issues, and the client install state were last observed 2026-07-26. Branch protection, rulesets and Actions secrets are not in this set. They were re-checked live on 2026-09-10 [EV-0131]. The carried-forward rows are cited with their 2026-07-26 date in place, and the assessment does not treat them as current |
...
| Security maturity | **Mixed, and the mix is the finding.** Restrictive hooks, permission-scoped workflows and an all-false action ceiling sit beside an unscanned dependency surface and a default branch still ungated on 2026-09-10 [EV-0131] | [EV-0121], [EV-0131], [EV-0038], [EV-0077] | V |
...
| Static analysis coverage | Partial, and the uncovered half is flow's shell. `dossier-tests.yml` installs shellcheck v0.11.0 and runs it at `-S warning` over dossier's 20 `bin/` and 5 hook scripts. No other workflow runs shellcheck [EV-0165]. CodeQL configures `actions` and `python` only [EV-0176]. flow's 17 `.sh` bin scripts and 14 hook scripts are therefore analysed by nothing [EV-0175], [EV-0129]. That is 31 of the 56 tracked shell files across the two plugins. AQ-0019 carries the open question | [EV-0165], [EV-0176], [EV-0175], [EV-0129] | V |
...
| Merge gating | `main` carries no branch protection and no repository rulesets, re-checked live at `e104483` on 2026-09-10 and confirming the 2026-07-26 reads [EV-0131], [EV-0016], [EV-0017]. Both CI results are therefore advisory. The re-check was run by the session operator, outside the agent action ceiling (AQ-0012) | [EV-0131], [EV-0016], [EV-0017] | V |
...
| Aspect | Finding | Evidence | State |
|---|---|---|---|
| Contributor concentration | One author identity on `main`, across 214 commits [EV-0163]. Across all refs, 480 commits carry two identities. The second is an assistant co-author trailer on 22 non-`main` commits, which the ledger records as not a third-party contributor [EV-0192] | [EV-0163], [EV-0192] | V |
| Ownership coverage | Every component has an owner. Every component has the same owner, and no component has a backup owner. The owner-of-record set is the ownership model in `00-control/terminology-and-ownership.md`, not a ledger row. The single-identity clause rests on [EV-0163] | [EV-0163] | V |
| Escalation | 6 of 6 decision domains have no recorded escalation path, and security disclosure handling is `unassigned` | [EV-0036], [EV-0163] | V |
| Review capability | With no branch protection, re-checked live on 2026-09-10 [EV-0131], and one author identity on `main` [EV-0163], no change in the inspected history was gated on a second reviewer | [EV-0131], [EV-0163] | I |
...
| Readability of the change surface | Of the 40 tracked `plugins/*/bin/` files, 37 are shell and 3 are Python, all three in flow [EV-0175]. Every artifact is text the operator can read before install | [EV-0044], [EV-0175] | V |
...
| Auditable execution surface | 40 plugin `bin/` files, 37 shell and 3 Python, with no binaries or bundles [EV-0175]. An installer can read what will run before trusting it | [EV-0175], [EV-0044] | V |
...
| Nothing gates `main` | Any change reaches every installer's machine as executable code with no check having to pass. `main` was still unprotected and ruleset-free on 2026-09-10 [EV-0131], and `autoUpdate` was on by default when last observed | A mistaken commit, or one compromised account | [EV-0131], [EV-0016], [EV-0017], [EV-0051] | V |
...
| Bus factor of one | One author identity across 214 commits on `main` [EV-0163], no third-party contributor established across any ref [EV-0192], no backup owner on any component, no escalation path | Any absence of one person | [EV-0163], [EV-0192], [EV-0036] | V |
| flow's shell is unanalysed while dossier's is linted | shellcheck covers dossier only [EV-0165] and CodeQL configures `actions` and `python` [EV-0176], so flow's 17 `.sh` bin scripts and 14 hook scripts are analysed by nothing [EV-0175], [EV-0129]. Those hooks run as blocking `PreToolUse` calls on other machines | A defect a shell linter would have caught, in code that runs on a contributor's machine | [EV-0165], [EV-0176], [EV-0175], [EV-0129] | V |
...
The manifest version check and its passing result [EV-0080], [EV-0081], [EV-0127]. The workflow set [EV-0123]. Licence text and declarations [EV-0019], [EV-0021]. The action ceiling [EV-0077].

The shellcheck step and CodeQL's configured language matrix [EV-0165], [EV-0176]. The live branch-protection re-check [EV-0131]. Commit and author-identity counts, per ref set [EV-0163], [EV-0192].

The absence of any scan artifact [EV-0121], [EV-0073]. The absence of any tracked dependency manifest, and PyYAML as an undeclared runtime requirement of flow [EV-0186], [EV-0187], [EV-0189].
...
That distribution is a git read rather than a registry publish [EV-0043]. That four of six in-tree entries carry no suite, from EV-0010 plus the submodule removal. That no change was gated on a second reviewer, from the live absence of protection [EV-0131] and the single identity on `main` [EV-0163].

### Unknowns

Whether the dependency surface holds any known vulnerability [EV-0121]. Whether dossier's automation works end to end (AQ-0002). Whether the pinned external trees differ materially from their tags (AQ-0006, AQ-0005). Whether the 22 commits carrying an assistant co-author identity change the inbound-rights position [EV-0192] (AQ-0007). Whether anyone installs and uses the product (AQ-0004). Whether the Python suite in the external source passes (AQ-0001).
...
| Ungated `main` on an auto-updating channel | **High** | No check has to pass, and installers receive whatever lands without acting. Still ungated on 2026-09-10 [EV-0131]. One compromised account reaches every operator machine | [EV-0131], [EV-0016], [EV-0017], [EV-0051] | Branch protection requiring both test workflows and one review |
...
| Bus factor of one | Medium-High | 8 published entries freeze on any absence of one person, and no installer can weigh this because it is not disclosed | [EV-0163], [EV-0192] | A named backup maintainer, or a stated single-maintainer status |
...
| 2 | Ungated `main` feeding an auto-updating channel | Any adopter inherits an unbounded supply-chain exposure | medium | none | [EV-0131], [EV-0016], [EV-0017] | Branch protection | Daniel Bentes |
| 3 | No security disclosure path | The first real finding becomes public | medium | none | [EV-0036] | `SECURITY.md` | Daniel Bentes |
| 4 | Bus factor of one | Continuity risk across all 8 entries | certain | high | [EV-0163], [EV-0192] | Backup maintainer, or disclosure | Daniel Bentes |
...
| 6 | flow's shell surface unanalysed | 31 of the 56 tracked shell files across the two plugins have no automated analysis [EV-0175], [EV-0129], and 14 of those 31 run as blocking `PreToolUse` hooks on other machines. Re-derived after shellcheck was found covering dossier: the narrowing did not move this position | medium | partial. dossier's 25 shell files are linted [EV-0165], flow's 31 are not [EV-0175], [EV-0129] | [EV-0165], [EV-0176], [EV-0175], [EV-0129] | Extend the shellcheck step to `flow-tests.yml` | Daniel Bentes |
...
| Extend shellcheck from dossier to flow | one step, copied from `dossier-tests.yml` [EV-0165], plus unknown fixes | The fix volume across flow's 31 shell files is unknown until it runs [EV-0175], [EV-0129] | Running it once | **low on the fix volume** |
...
| 5 | Is there a second person who can release, merge and answer a security report? | Continuity across 8 published entries | A named backup, or a public statement that there is none | [EV-0163] |
...
| 60 | Extend the shellcheck step from `dossier-tests.yml` to `flow-tests.yml`, and pin the CI actions to shas | Ranks 6, 10 | Closes the remaining analysis gap [EV-0165] while flow's shell surface is 31 files [EV-0175], [EV-0129], and the supply-chain gap | Daniel Bentes |
