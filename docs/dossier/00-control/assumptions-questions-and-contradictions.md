...
| AQ-0010 | assumption | Assessment on an unmerged feature branch is representative of the project | This package was produced at `fd884b2` on `fix/issue-131-round-5-containment`, not on `main` | State it in every header via `project-version`, and treat `main`-only claims as out of scope | Re-run after merge | Daniel Bentes | no | all | open | Version 4.7.0 and the entire dossier plugin exist only on this branch (EV-0033) |
