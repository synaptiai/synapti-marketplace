...
Two numbers dominate this document and both are real: 1022 passing assertions in flow, 1034 in dossier, zero failures at `d0fa737` [EV-0008], [EV-0009]. They are also narrower than they look, in three specific ways that this document exists to make explicit — they cover 2 of 7 in-tree plugins, they test structure rather than behaviour, and no check is required for merge.
...
| dossier | 1034 assertions | assertion count | same | Its headline capability has never run (AQ-0002) | The plugin's central claim is unverified | [EV-0009], [EV-0045] |
...
This table is the core finding of the document. The project has genuinely good tests and **no gate anywhere that stops a change**. The 2,056 assertions are information, not enforcement.
...
| Review policy | Pull requests used by habit — 62 merged against 194 commits | **nothing** | [EV-0016], [EV-0050] |
...
| Deployment frequency | 194 commits to `main` over ~7 months, each of which is a deployment | 2025-12-19 to 2026-07-25 | git | V | [EV-0034] |
| Merged pull requests | 62 | full history | GitHub | V | [EV-0050] |
| Test assertions executed per run | 2,056 across both suites | per CI run | `tests/run.sh` | V | [EV-0008], [EV-0009] |
...
| `plugins/dossier/tests/run.sh` | Verify dossier's suite passes | macOS 25.5, bash | 2026-07-26 | pass | `TOTAL pass=1034 fail=0` |
