...
Two numbers dominate this document and both are real: 1022 passing assertions in flow, 1190 in dossier, zero failures at `d8c10fa` [EV-0008], [EV-0009]. They are also narrower than they look, in three specific ways that this document exists to make explicit — they cover 2 of 7 in-tree plugins, they test structure rather than behaviour, and no check is required for merge.
...
| dossier | 1190 assertions | assertion count | same | Its headline capability has never run (AQ-0002) | The plugin's central claim is unverified | [EV-0009], [EV-0045] |
...
This table is the core finding of the document. The project has genuinely good tests and **no gate anywhere that stops a change**. The 2,212 assertions are information, not enforcement.
...
| `plugins/dossier/tests/run.sh` | Verify dossier's suite passes | macOS 25.5, bash | 2026-07-26 | pass | `TOTAL pass=1190 fail=0` |
